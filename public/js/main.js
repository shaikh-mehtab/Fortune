(function ($) {
  ...
  $(document).on("click", ".submit", function () {
    ...
    if (!error && actionURL !== "" && actionURL !== undefined) {
        ...
        var counter = formObj.find('button.submit').val(); 

        var formData = new FormData(formObj[0]); // Use FormData to include file inputs

        $.ajax({
            type: "POST",
            url: actionURL,
            data: formData,
            contentType: false, // Important: Do not set content type, it will be set automatically
            ...
            success: function (result) {
                ...
                if (redirectVal !== "" && redirectVal !== undefined) {
                    ...
                    formObj
                        .find("input[type=text],input[type=url],input[type=email],input[type=tel],input[type=password],textarea")
                        .each(function () {
                            $(this).val("");
                ...
                setTimeout(function() {
                  $('#exampleModal' + counter).find('.modal-content').fadeOut('slow', function() {
                      $('#exampleModal' + counter).modal('hide');
                  });
              }, 2000);
